...
import { roleLabel } from "@/lib/marketplace";
...
export default async function AdminProvidersPage({searchParams}:{searchParams:Promise<{approved?:string}>}){
 ...
 return <div className="admin-main"><div className="row"><div><h1>Provider applications</h1><p className="muted">Review SASL Tutors and SASL Interpreters.</p></div><Link href="/admin" className="btn secondary">Dashboard</Link></div>{q.approved==="1"?<p className="admin-success" role="status"><strong>Provider approved successfully.</strong> The provider is now listed below.</p>:null}<div className="admin-table" style={{marginTop:20}}>{(providers||[]).map(p=>{const r=(roles||[]).filter(x=>x.provider_id===p.id&&(x.role==="deaf_tutor"||x.role==="interpreter")); const v=(verifications||[]).filter(x=>x.provider_id===p.id);return <Link href={`/admin/providers/${p.id}`} className="admin-row" key={p.id}><div><strong>{p.public_display_name||"Unnamed applicant"}</strong><small>{r.length?r.map(x=>roleLabel(x.role)).join(" · "):"No role selected"}</small></div><div><span className={`status ${p.status==="approved"?"approved":""}`}>{p.status}</span><small>{v.filter(x=>x.state==="pending").length} verification item(s) pending</small></div><strong>Review →</strong></Link>})}{!providers?.length?<div className="card">No provider applications yet.</div>:null}</div></div>;
}
