...
export default async function MarketplacePage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  const roleLocked = Boolean(q.subjectName && role);
  ...
  return <div className="shell"><header className="topbar"><Link href={subject?"/subjects":"/"}>←</Link><strong>{q.subjectName || "Find a provider"}</strong><button className="help-btn" aria-label="Open SASL help">?</button></header>
    <main className="main">
      ...
      <form className="market-filters" method="get">
        ...
        {roleLocked ? <><input type="hidden" name="role" value={role || ""}/><label>Provider type<div className="field readonly-field">{roleLabel(role || "")}</div></label></> : <label>Provider type<select className="field" name="role" defaultValue={role || ""}><option value="">Any provider type</option><option value="deaf_tutor">SASL Tutor</option><option value="interpreter">SASL Interpreter</option></select></label>}
