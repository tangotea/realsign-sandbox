...
import { PublicProvider, money, roleLabel } from "@/lib/marketplace";import ReportReviewButton from "@/components/meet/ReportReviewButton";
  ...
  return <div className="shell"><header className="topbar"><Link href="/marketplace">←</Link><strong>{p.display_name}</strong><button className="help-btn" aria-label="Open SASL help">?</button></header>
    <main className="main">
      <section className="provider-profile-head">
        ...
        <div className="tag-row">{p.roles.map(r=><span className="pill" key={r.role}>{roleLabel(r.role)}</span>)}</div>
      ...
      <section className="card"><h2>About me</h2><p>{p.introduction_text || "This provider has not added written introduction text yet."}</p><h3>Languages I use</h3><p>{p.languages.map(l=>l.name).join(" · ") || "Not listed"}</p></section>
      ...
      <section className="card"><h2>Services</h2><div className="service-list">{p.services.map(s=><div className="service-row" key={s.id}><div><strong>{s.title}</strong><small>{s.duration_min} minutes · {roleLabel(s.provider_role)}</small></div><div className="service-action"><strong>{money(s.price_cents)}</strong>{s.provider_role==="interpreter"?<>{s.remote?<Link className="mini-btn" href={`/providers/${p.id}/book?service=${s.id}`}>Remote times</Link>:null}{s.in_person?<Link className="mini-btn" href={`/providers/${p.id}/request`}>Request in person</Link>:null}</>:<Link className="mini-btn" href={`/providers/${p.id}/book?service=${s.id}`}>View times</Link>}</div></div>)}</div></section>
