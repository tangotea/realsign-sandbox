...
export default async function ProviderProfilePage({ params, searchParams }: { params: Promise<{id:string}>; searchParams: Promise<{role?:string}> }) {
  ...
  return <div className="shell"><header className="topbar"><Link href="/marketplace">←</Link><strong>{p.display_name}</strong><button className="help-btn" aria-label="Open SASL help">?</button></header>
    <main className="main">
      ...
      <section className="card"><h2>About me</h2><p>{p.introduction_text || "This provider has not added written introduction text yet."}</p><h3>Languages I use</h3><p>{p.languages.map(l=>languageLabel(l.name, roleContext)).join(" · ") || "Not listed"}</p></section>
      <section className="card"><h2>Services</h2><div className="service-list">{visibleServices.map(s=><div className="service-row" key={s.id}><div><strong>{serviceLabel(s)}</strong><small>{s.duration_min} minutes · {roleLabel(s.provider_role)}</small>{serviceDetailLabel(s)?<small>{serviceDetailLabel(s)}</small>:null}</div><div className="service-action"><strong>{money(s.price_cents)}</strong>{s.provider_role==="interpreter"&&s.in_person?<Link className="mini-btn" href={`/providers/${p.id}/request`}>Request in person</Link>:null}</div></div>)}</div></section>
