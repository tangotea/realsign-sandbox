...
export default function ProviderApplication() {
  ...
  const refresh = useCallback(async () => {
    ...
    const [profileRes, roleRes, subjectRes, chosenRes, serviceRes, verifyRes, settingsRes] = await Promise.all([
      ...
      supabase.from("subjects").select("id,name,phase,min_grade,max_grade").eq("active", true).order("display_order"),
    ...
    setSelectedSubjects(new Set((chosenRes.data || []).map((r: { subject_id: string }) => r.subject_id)));
  ...
  return <div className="stack">
    ...
    <section className="card">
      <h2>4. Subjects</h2><p>Choose the subjects you genuinely want to tutor or teach.</p>
      {Array.from(new Set(subjects.map(s=>s.phase))).map(phase => <details key={phase} className="subject-group" open={phase==="Senior"}><summary>{phase}</summary><div className="checklist">{subjects.filter(s=>s.phase===phase).map(s=><label className="check" key={s.id}><input disabled={!editable} type="checkbox" checked={selectedSubjects.has(s.id)} onChange={()=>{const next=new Set(selectedSubjects); next.has(s.id)?next.delete(s.id):next.add(s.id); setSelectedSubjects(next);}}/><span><strong>{s.name}</strong><small>{s.min_grade===0?"Grade R":s.min_grade ? `Grade ${s.min_grade}` : ""}{s.max_grade && s.max_grade!==s.min_grade ? `–${s.max_grade}`:""}</small></span></label>)}</div></details>)}
    ...
    <section className="card">
      ...
      {editable ? <form className="form-grid" onSubmit={async e=>{e.preventDefault(); await createService(e.currentTarget);}}>
        ...
        <label className="span2">Service title<input className="field" name="title" placeholder="e.g. Grade 8 Mathematics Tutoring" required /></label>
