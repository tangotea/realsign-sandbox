...
export default function BookingPicker({provider,service,services}:{provider:PublicProvider;service:Service;services?:Service[]}){
  ...
  const [learnerFor,setLearnerFor]=useState("myself"); const [firstName,setFirstName]=useState(""); const [note,setNote]=useState("");
  ...
  async function continueBooking(){ if(!selected)return; setBusy(true); setMessage(""); const {data:auth}=await supabase.auth.getUser(); if(!auth.user){const nextPath=`${window.location.pathname}${window.location.search}`;router.push(`/sign-in?next=${encodeURIComponent(nextPath)}`);return;} const {data:identity}=await supabase.from("user_identity_verifications").select("state").eq("user_id",auth.user.id).maybeSingle(); if(identity?.state!=="approved"){setBusy(false);setMessage("Identity verification is required before booking.");return;} const {data,error}=await supabase.rpc("create_booking_hold",{p_service_id:selectedService.id,p_start_at:selected.start_at,p_learner_for:learnerFor,p_learner_first_name:firstName||null,p_learner_grade:null,p_learner_note:note||null}); if(error){setBusy(false);setMessage(error.message);return;} router.push(`/checkout/${data.hold_id}`); }
  ...
  return <div className="stack"><section className="card"><div className="page-heading"><h1>Book {provider.display_name}</h1><HelpButton slug="booking-time" label="Booking time help" size="regular" fallbackText="Choose an available date and time, select the lesson or interpreting service you want, then continue to confirm and pay." /></div>{serviceChoices.length>1?<div className="service-picker" role="group" aria-label="Choose lesson length">{serviceChoices.map(s=><button key={s.id} type="button" className={s.id===selectedService.id?"selected":""} onClick={()=>{setServiceId(s.id);setSelected(null);}}><strong>{s.duration_min} min</strong><small>{money(s.price_cents)} · {serviceLabel(s)}</small></button>)}</div>:null}<p><strong>{serviceLabel(selectedService)}</strong><br/>{selectedService.duration_min} minutes · {money(selectedService.price_cents)}{serviceDetailLabel(selectedService)?<><br/><span className="muted">{serviceDetailLabel(selectedService)}</span></>:null}</p><div className="calendar-head"><button className="mini-btn" disabled={!canGoBack} onClick={()=>setVisibleMonth(addMonths(visibleMonth,-1))}>‹</button><strong>{monthLabel(visibleMonth)}</strong><button className="mini-btn" disabled={!canGoNext} onClick={()=>setVisibleMonth(addMonths(visibleMonth,1))}>›</button></div><div className="calendar-legend"><span><b className="legend-dot available"></b>Tap to book</span><span><b className="legend-dot unavailable"></b>Unavailable</span><span><b className="legend-x">X</b>Closed</span></div><div className="booking-calendar" aria-label="Choose a day">{["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map(day=><div className="calendar-weekday" key={day}>{day}</div>)}{calendarDays.map(day=>{if(day.outside)return <div className="calendar-day empty" key={day.date}/>;const slots=monthSlots[day.date]||[];const isSelected=date===day.date;const hasSlots=slots.length>0;return <button key={day.date} type="button" disabled={day.past||!hasSlots} className={`calendar-day ${isSelected?"selected":""} ${hasSlots?"has-slots":"no-slots"} ${day.past?"past":""}`} onClick={()=>{setDate(day.date);setSelected(null);}}><span className="day-number">{day.day}</span>{day.past?<span className="day-status">Past</span>:hasSlots?<span className="day-pill"><strong>{slots.length===1?timeLabel(slots[0],zone):`${slots.length} slots`}</strong><small>Tap to book</small></span>:<span className="day-status"><strong>X</strong><small>Unavailable</small></span>}</button>})}</div><div className="selected-day"><strong>Available times for {date}</strong>{busy?<p className="muted">Checking availability...</p>:selectedSlots.length?<div className="slot-grid">{selectedSlots.map(s=><button key={s.start_at} className={`slot-btn ${selected?.start_at===s.start_at?"selected":""}`} onClick={()=>setSelected(s)}><strong>{timeLabel(s,zone)}</strong><small>Choose time</small></button>)}</div>:<p className="muted">No available times on this date. Choose a green day.</p>}</div></section>
  {selected?<section className="card"><h2>Who is this session for?</h2><label className="check"><input type="radio" checked={learnerFor==="myself"} onChange={()=>setLearnerFor("myself")}/> Myself</label><label className="check"><input type="radio" checked={learnerFor==="child_or_dependent"} onChange={()=>setLearnerFor("child_or_dependent")}/> My child / dependent</label><label className="check"><input type="radio" checked={learnerFor==="assisted_person"} onChange={()=>setLearnerFor("assisted_person")}/> Someone I am assisting</label>{learnerFor!=="myself"?<label>Learner first name<input className="field" value={firstName} onChange={e=>setFirstName(e.target.value)}/></label>:null}<label>Anything useful for the tutor or interpreter? (optional)<textarea className="field" rows={3} value={note} onChange={e=>setNote(e.target.value)}/></label><button className="btn" disabled={busy} onClick={continueBooking}>Continue to checkout</button>{message?<p className="notice">{message} {message.includes("Identity")?<a href="/profile/identity"><strong>Open identity verification →</strong></a>:null}</p>:null}</section>:null}</div>;
}
