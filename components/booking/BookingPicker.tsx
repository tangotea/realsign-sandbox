...
export default function BookingPicker({provider,service}:{provider:PublicProvider;service:Service}){
  ...
  const maxMonthOffset=service.provider_role==="interpreter"?3:2;
  ...
  useEffect(()=>{setVisibleMonth(currentMonth);setDate(today);},[currentMonth,today,service.id]);
  const calendarDays=useMemo(()=>monthDays(visibleMonth,today),[visibleMonth,today]);
  const selectedSlots=monthSlots[date]||[];
  useEffect(()=>{let active=true;(async()=>{setBusy(true);setMessage("");const results=await Promise.all(calendarDays.filter(d=>!d.outside&&!d.past).map(async d=>{const {data,error}=await supabase.rpc("get_service_slots",{p_service_id:service.id,p_date:d.date});return [d.date,{slots:(data||[]) as Slot[],error:error?.message||""}] as const;}));if(!active)return;const next:Record<string,Slot[]>={};let errorMessage="";for(const [day,result] of results){next[day]=result.slots;if(result.error&&!errorMessage)errorMessage=result.error;}setMonthSlots(next);setMessage(errorMessage);setSelected(null);setBusy(false);})();return()=>{active=false};},[calendarDays,service.id,supabase]);
